...
# ---- Optional Persian (Jalali) date parsing ----
# If persiantools is installed, we'll accept Jalali dates like 1403/06/15 18:30
try:
...
# ---- Third-party SDKs (provide at runtime) ----
try:
    from rubpy import Client  # type: ignore
    from rubpy.types import Update  # type: ignore
except Exception:  # pragma: no cover
    # Lightweight shims for type hints if rubpy isn't available at lint time
...
CHANNEL_GUID = os.getenv("CHANNEL_GUID")  # optional, used if you need a default channel
...
# Configure Gemini model (if available)
GENERATION_CONFIG = {
    "temperature": 0.9,
    "top_p": 1,
    "top_k": 1,
...
if HAS_GENAI and GEMINI_API_KEY:
    ...
    _model = genai.GenerativeModel("gemini-pro", generation_config=GENERATION_CONFIG)
else:
    _model = None
...
async def generate_response(prompt: str, user_type: str) -> str:
    ...
    try:
        # Adjust style/limits based on user type
        system_preamble = (
...
# ======================================
class DBManager:
    """Manages database interactions: users, admins, ads, channel requests."""
    ...
    def make_vip(self, guid: str, duration_days: int):
        """Upsert VIP; if duration_days == 0, remove VIP immediately."""
    ...
    def get_active_ads(self):
        with self.get_connection() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT * FROM ads WHERE run_at <= ? ORDER BY run_at DESC",
                (datetime.now().timestamp(),),
            )
            return cur.fetchall()
...
class AIBot:
    ...
    async def _safe_send(self, target_guid: str, text: str):
        try:
            # Keep messages within a safe length
            MAX_LEN = 4000
            if len(text) <= MAX_LEN:
    ...
    def _parse_datetime(self, text: str) -> float:
        """Parse datetime string. Supports Jalali if persiantools installed; otherwise Gregorian.
        Returns Unix timestamp.
        Accepted formats (examples):
          - 1403/06/15 18:30 (Jalali, if persiantools available)
    ...
    async def handle_message(self, message: Update):
        try:
            ...
            # Register/update user activity early
            try:
                if author_guid:
                ...
                if state == 'add_vip_duration':
                    ...
                    if getattr(message, 'reply_to_message_id', None):
                        ...
                        if replied and replied[0].get('author_guid'):
                            ...
                            await self._safe_send(object_guid, "لطفا تعداد روزهایی که کاربر باید VIP باشد را به صورت یک عدد وارد کنید.")
                        else:
                    ...
                    try:
                        ...
                        await self._safe_send(object_guid, "فرمت تاریخ و ساعت اشتباه است. مثال: 1403/06/15 18:30 (شمسی) یا 2025/09/03 18:30 (میلادی).")
                        return
            ...
            await self._safe_send(getattr(message, 'object_guid', author_guid), "یک خطای ناشناخته رخ داد. لطفا دوباره تلاش کنید.")
        ...
        try:
            ...
            if admin_level != -1:
                if data == 'vip_manage':
                    ...
                    await self._safe_send(sender_guid, "لطفاً نام کاربری (یوزرنیم) ادمین جدید را بدون @ وارد کنید. مثال: username")
                    self.admin_states[sender_guid] = {'state': 'waiting_for_admin_username'}
                    return
                elif data == 'remove_sub_admin' and admin_level == 1:
                    await self._safe_send(sender_guid, "لطفا روی پیام ادمین فرعی که می‌خواهید حذف کنید، Reply بزنید و این پیام را برای ربات ارسال کنید.")
                    self.admin_states[sender_guid] = {'state': 'waiting_for_admin_to_remove'}
                    return

            # User callbacks (non-admin)
            if data == 'about':
                response = (
    ...
    async def handle_ai_command(self, message: Update, text: str, user_data):
        ...
        # Ensure user exists; if not, fetch again (defensive)
        if not user_data:
            user_data = self.db_manager.get_user(getattr(message, 'author_guid', ''))
            if not user_data:
        ...
        if is_vip and (vip_expiry is None or datetime.now().timestamp() > float(vip_expiry)):
            # downgrade
            self.db_manager.make_vip(getattr(message, 'author_guid', ''), 0)
            is_vip = False

        prompt = text.replace("/ai", "", 1).strip()
        if not prompt:
            await self._safe_send(getattr(message, 'object_guid', None), "لطفا یک سوال بعد از /ai بپرسید.")
            return
    ...
    async def handle_summarize_command(self, message: Update, text: str, user_data):
        ...
        if not prompt:
            await self._safe_send(getattr(message, 'object_guid', None), "لطفا متنی را برای خلاصه‌سازی بعد از /summarize وارد کنید.")
            return
    ...
    async def handle_admin_login(self, message: Update, text: str, user_data):
        admin_level = self.db_manager.get_admin_level(getattr(message, 'author_guid', ''))
        if admin_level != -1:
            ...
            try:
                ...
                if ads:
                    ...
                    for ad in ads:
                        ...
                        # Remove the ad after distribution
                        with self.db_manager.get_connection() as conn:
    ...
    def run(self):
        try:
            ...
            # Start scheduler in a background thread to avoid event loop conflicts
            import threading
            threading.Thread(target=lambda: asyncio.run(self.run_ads_scheduler()), daemon=True).start()

            # Start rubpy client's own loop
...
if __name__ == "__main__":
    if not all([AUTH_KEY, MASTER_ADMIN_GUID, MASTER_PASSWORD, SUB_ADMIN_PASSWORD]):
        ...
        try:
            bot = AIBot(
                AUTH_KEY,
                CHANNEL_GUID,  # keep None if you don't need it
                MASTER_ADMIN_GUID,
                MASTER_PASSWORD,
                SUB_ADMIN_PASSWORD,
            )
